...
namespace Task19
{
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("Укажите тип процессора");
            string processorModel = Console.ReadLine();
            List<Computer> computers = (from c in listComputer
                                         where c.ProcessorModel == processorModel
                                         select c).ToList();
            foreach (Computer c in computers)
                Console.WriteLine($"{c.Id} {c.ProcessorModel} ");           
            Console.ReadKey();
        }
    }
}
