...
namespace Task19._5
{
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            double max = listComputer.Max(m => m.Price);

            List<Computer> computers1 = (from c in listComputer
                                        where c.Price == min
                                        select c).ToList();
            List<Computer> computers2 = (from c in listComputer
                                        where c.Price == max
                                        select c).ToList();

            foreach (Computer c in computers1)
                Console.WriteLine($"{c.Id} {c.Price} ");
